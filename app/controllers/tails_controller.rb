class TailsController < ApplicationController
  ...
  before_action :set_tail, only: [:show, :edit, :update, :destroy, :vote]
  ...
  def create
    ...
    @tail.user_id = current_user.id

    respond_to do |format|
      if @tail.save
        format.html { redirect_to brain_path(@tail.brain)} #notice: 'Tail was successfully created.' }
  ...
  def vote
    if !current_user.liked? @tail
      @tail.liked_by current_user
    elsif current_user.liked? @tail
      @tail.unliked_by current_user
    end
  ...
  private
    ...
    def tail_params
      params.require(:tail).permit(:description, :user_id, :brain_id)
    end
end
